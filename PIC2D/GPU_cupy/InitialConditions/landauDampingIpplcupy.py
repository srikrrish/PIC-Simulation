...
def f(x,alpha,kd,u):
    return (x  + (alpha * (cp.sin(kd[:,None] * x[:,]) / kd[:,None])) - u)

def fprime(x,alpha,kd,u):
    return(1 + (alpha * cp.cos(kd[:,None] * x[:,])))
...
def Newton1d(xi, alpha, kd, u):
    ...
    k=0
    x=0
    while (k <= max_iter) and (cp.max(cp.abs(f(xi,alpha,kd,u))) > tol):
...
def InvTransSampling(alpha,k,L,N):
    xp = cp.zeros([2, N])
    u0 = cp.random.rand(2, N)
    #Lc = L.get()
    #kc = k.get()
    u =  L[:,None] * u0[:,]
    x = u / (1+alpha)
    xp,niter = Newton1d(x,alpha,k,u)
    #for i in range(N):
    #    print(i)
    #    for d in range(2):
    #        u =  Lc[d] * u0[d, i]
...
DT = 0.005  # Length of a time step
T = 20
